...
function updateTodoStateOnLocalStorage(todoTitle, isCompleted) {
  ...
  todoItems.forEach((todo) => {
    todo.title === todoTitle ? (todo.isFinished = isCompleted) : null;
  });
...
function removeTodoFromLocalStorage(todoTitle) {
  ...
  todoItems.forEach((todo, index) => {
    todo.title === todoTitle ? todoItems.splice(index, 1) : todoItems;
  });
...
function createTodo(todoItem, isCompleted) {
  ...
  isCompleted === true
    ? (todoListItem.querySelector("input").checked = "True")
    : null;
...
function clearCompletedTodo() {
  const clearCompletedBtn = document.querySelector(
    ".clear-completed-todo__btn"
  );

  clearCompletedBtn.addEventListener("click", (e) => {
    document
      .querySelectorAll(".todo-list__item--completed")
      .forEach((todo) => todoRemover(todo));
  });
}
...
function todoStateModifier(todo, parentElement) {
  todo.checked
    ? parentElement.classList.add("todo-list__item--completed")
    : parentElement.classList.remove("todo-list__item--completed");
}
...
function todoStateInitializer() {
  getTodosFromLocalStorage().forEach((todo) =>
    createTodo(todo.title, todo.isFinished)
  );
